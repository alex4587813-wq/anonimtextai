...
def build_document():
    ...
    steps = [
        ...
        ("Проверьте правила", "При необходимости включите категории, добавьте исключения или обязательные замены."),
    ...
    definitions = [
        ...
        ("Настройки правил", "Управляют категориями, исключениями и обязательными заменами."),
    ...
    doc.add_heading("4.3. Обязательная замена организаций", level=2)
    doc.add_paragraph(
        "Термины из этого списка всегда заменяются как организации, даже если слово "
        "встречается без поясняющего контекста. По умолчанию добавлены:"
    )
    for item in ["СИЛА", "Платформикс", "Базовые решения"]:
        add_bullet(doc, item, bullet_num_id)

    doc.add_paragraph(
        "Чтобы добавить организацию, введите название в поле и нажмите «Добавить». "
        "Чтобы прекратить обязательную замену, удалите термин значком ×."
    )
    ...
    add_callout(
        doc,
        "Порядок",
        "обязательная замена имеет высший приоритет. Она срабатывает, даже если категория «Организации» выключена или то же слово добавлено в список исключений.",
        caution=True,
    )
    add_table(
        doc,
        ["Настройка", "Результат"],
        [
            ("Термин в обязательной замене", "Всегда заменяется на [COMPANY_XXX]"),
    ...
    for item in [
        ...
        "при работе с новыми типами данных добавляйте необходимые обязательные замены;",
    ...
    for item in [
        "обработка выполняется локально на Mac;",
    ...
    doc.add_paragraph(
        ...
        "название проекта. Для повторяющихся корпоративных названий используйте список обязательной замены."
    )
    ...
    faq = [
        ...
        ("Что сильнее: исключение или обязательная замена?", "Обязательная замена всегда имеет высший приоритет."),
